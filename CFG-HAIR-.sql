...
-- Create a store function that can be applied to a query in your DB 
DELIMITER //
CREATE FUNCTION product_fullname(Product_Name VARCHAR(75),product_brand VARCHAR (50))
RETURNS VARCHAR(100) DETERMINISTIC
RETURN CONCAT(Product_Name,' ',product_brand);
END //
SELECT product_id, product_fullname(product_name, product_brand) AS pfn
FROM retail_store;
